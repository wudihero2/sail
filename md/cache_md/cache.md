...
Sail 的 Moka 加了 TTL 作為保底機制（預設 1800 秒 = 30 分鐘），
...
🔸 為什麼 DataFusion 的 is_valid_for() 不能完全防止過期

使用者可能以為 DataFusion 每次都會比對 size & last_modified，所以不會有過期問題。
但 is_valid_for() 比對的 「current ObjectMeta」 從哪來？

```
Phase 0: list directory → 拿到 Vec<ObjectMeta>   ← current_meta 的來源
Phase 1: 對每個檔案，用 ObjectMeta 查 metadata cache
         → is_valid_for(current_meta)
```

如果 Phase 0 每次都真的打 S3 list()，那 is_valid_for() 拿到的一定是最新的 ObjectMeta，不會過期。
但如果 listing cache HIT 了，Phase 0 回傳的是「cache 住的舊 ObjectMeta」：

```
Phase 0: listing cache HIT → 回傳 cache 住的 Vec<ObjectMeta>（可能是 30 分鐘前的）
Phase 1: is_valid_for(舊的 ObjectMeta) → 比對一致 → 以為沒變 → 過期資料
```

is_valid_for() 本身沒有 bug，它忠實地比對你給它的東西。問題出在上游 — listing cache 給它的 ObjectMeta 可能是舊的。

整條信任鏈：

```
listing cache TTL → 決定 ObjectMeta 的新鮮度
                  → 決定 is_valid_for() 的輸入
                  → 決定 metadata cache 的正確性
                  → 決定 data cache 的正確性

沒有 listing cache → is_valid_for() 永遠拿到最新 ObjectMeta → 永遠正確
有 listing cache   → is_valid_for() 可能拿到過期 ObjectMeta → 最大延遲 = listing TTL
```

DataFusion 預設沒有 listing cache，所以預設不會有過期問題。
是 Sail 自己加了 MokaFileListingCache 才引入了這個過期窗口。

🔸 為什麼 Sail 要加 MokaFileListingCache

PR #709（2025-08-05，作者 shehabgamin）引入了 MokaFileListingCache。
對應 issue #516 描述的問題：

```
問題：plan resolver 每次查詢都會 list 所有檔案 + 讀每個 Parquet 檔案的 metadata。
      當資料在 S3 且有大量 partition 時，list directory 非常慢。
解法：加 listing cache，同一個目錄只 list 一次，後續查詢直接用 cache。
```

預設設定（application.yaml:284-316）：

```
execution.file_listing_cache.type       = "none"   ← 預設關閉
execution.file_listing_cache.ttl        = 1800     ← 30 分鐘
execution.file_listing_cache.max_entries = 10000
```

注意：listing cache 預設是 "none"（關閉的）。
使用者要手動設成 "global" 或 "session" 才會啟用。

TTL 為什麼是 1800 秒（30 分鐘）：PR 和 issue 裡沒有特別討論這個數字的選擇。
issue #516 的態度是：假設檔案在 session 期間不會變，如果變了「使用者可以重啟 session」。
30 分鐘是 batch analytics 場景的保守選擇 — 對多數 ETL pipeline 來說，
資料更新頻率遠低於 30 分鐘。

```
// crates/sail-session/src/runtime.rs:113-135
fn create_file_listing_cache(&mut self) -> Arc<dyn ListFilesCache> {
    let ttl = self.config.execution.file_listing_cache.ttl;           // 1800
    let max_entries = self.config.execution.file_listing_cache.max_entries; // 10000
    match &self.config.execution.file_listing_cache.r#type {
        CacheType::None => {
            // 預設走這裡：max_capacity = 0，等於不 cache
            Arc::new(MokaFileListingCache::new(ttl, Some(0)))
        }
        CacheType::Global => {
            // 全域共用一個 cache instance
        ...
        CacheType::Session => {
            // 每個 session 各自一個 cache instance
            Arc::new(MokaFileListingCache::new(ttl, max_entries))
        }
    }
}
...
CacheType::None 的做法很巧妙：不是不建 cache，而是建一個 max_capacity = 0 的 cache，
這樣 DataFusion 的 CacheManager 拿到的不是 None，而是一個「永遠 miss」的 cache，
不需要改 DataFusion 的 if-let 邏輯。
